...
public:
	...
	*/
	...
	/**
	*	@brief	특정한 인덱스의 아이템을 가져옴.
	*	@return	성공 시 true, 인덱스를 벗어나거나 하여 실패 시 false.
	*/
...
// 입력받은 아이템을 데이터에서 찾아내어 전부 삭제한다.
template <typename T>
int DoublyLinkedList<T>::DeleteAll(T item) {
	if (IsEmpty()) {
		return 0;
	}
...
template<typename T>
bool DoublyLinkedList<T>::GetItemAt(T &item, const int index) {
